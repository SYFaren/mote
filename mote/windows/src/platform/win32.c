/* mote/mote/windows — developer: SYFaren — Win32, double-buffer, fully static */
#include "platform.h"
#include "common.h"
#include "utf8.h"
...
struct Plat {
  ...
  bool focused;
  bool caret_on;
  bool caret_shown; /* our ShowCaret/HideCaret nest tracking */
  bool font_owned;
  int caret_h;
  int wheel_acc;
  unsigned high_surr;
...
static void map_vk(WPARAM vk, bool ctrl, bool shift, PlatEvent *ev) {
  ev->type = PE_KEY;
  ev->ctrl = ctrl;
  ev->shift = shift;
  ev->key = PK_NONE;
...
static LRESULT CALLBACK wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
  ...
  case WM_SETFOCUS:
    ...
    p->caret_shown = false; /* CreateCaret leaves caret hidden */
    return 0;
  case WM_KILLFOCUS:
    p->focused = false;
    DestroyCaret();
...
Plat *plat_create(const char *title, int w, int h) {
  ...
  p->font = CreateFontW(-16, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
...
static HBRUSH brush_for(Plat *p, uint32_t rgb) {
  int i;
  HBRUSH br;
  for (i = 0; i < p->br_n; i++) {
...
bool plat_set_caret(Plat *p, int x, int y, int h, bool on) {
  ...
  return true; /* OS caret — skip software draw */
}
...
bool plat_clipboard_set(Plat *p, const char *s, size_t n) {
  ...
  if (!OpenClipboard(p->hwnd)) return false;
  EmptyClipboard();
