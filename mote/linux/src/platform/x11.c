/* mote/mote/linux — developer: SYFaren — X11 + pixmap double-buffer */
#include "platform.h"
#include "common.h"
#include "utf8.h"
...
static unsigned long rgb_pixel(Plat *p, uint32_t rgb) {
...
Plat *plat_create(const char *title, int w, int h) {
  ...
  p->font = XLoadQueryFont(p->dpy,
                           "-misc-fixed-medium-r-normal--15-*-*-*-*-*-iso10646-1");
  if (!p->font)
    p->font = XLoadQueryFont(p->dpy,
                             "-*-*-medium-r-normal--14-*-*-*-*-*-iso10646-1");
  if (!p->font) p->font = XLoadQueryFont(p->dpy, "9x15");
  if (!p->font) p->font = XLoadQueryFont(p->dpy, "fixed");
  if (!p->font) p->font = XLoadQueryFont(p->dpy, "6x13");
...
static void map_key(KeySym ks, unsigned state, PlatEvent *ev) {
  ...
  case XK_Tab: ev->key = PK_TAB; break;
