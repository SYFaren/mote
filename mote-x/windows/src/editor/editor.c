...
static void view_invalid(Doc *d) { d->row0_valid = false; }

static const Theme *th(Editor *e) { return theme_get(e->theme_id); }

static uint32_t hl_color(const Theme *t, HlKind k) {
  switch (k) {
...
static bool can_edit(const Doc *d) { return !d->readonly; }

static bool push_delete(Editor *e, Doc *d, size_t pos, size_t n) {
  char *t;
  size_t i;
  bool has_nl = false;
  if (!n || !can_edit(d)) return false;
  t = (char *)malloc(n);
  if (!t) {
  ...
  if (!undo_push(&d->undo, U_DELETE, pos, t, n, false)) {
    free(t);
    set_status(e, "out of memory");
    return false;
  }
...
static bool push_insert(Editor *e, Doc *d, size_t pos, const char *s, size_t n,
                        bool coalesce) {
  size_t i;
  bool has_nl = false;
  if (!n || !s || !can_edit(d)) return false;
  for (i = 0; i < n; i++) {
    if (s[i] == '\n') {
      has_nl = true;
      break;
...
static void insert_autoclose(Editor *e, Doc *d, char open, char close) {
  ...
  if (has_sel(d)) {
    ...
    pair[0] = open;
    if (!push_insert(e, d, lo, pair, 1, false)) return;
    hi += 1;
    pair[0] = close;
    if (!push_insert(e, d, hi, pair, 1, false)) return;
    d->caret = lo + 1;
    clear_sel(d);
    sync_caret_rc(d);
    ensure_visible(e, d);
  ...
  pair[0] = open;
  pair[1] = close;
  if (!push_insert(e, d, d->caret, pair, 2, false)) return;
  d->caret += 1;
  clear_sel(d);
  sync_caret_rc(d);
  ensure_visible(e, d);
...
static void move_caret(Editor *e, Doc *d, size_t np, bool keep_sel) {
  d->caret = np;
  clamp_caret(d);
...
static void move_vert(Editor *e, Doc *d, int dy, bool keep_sel) {
...
static void recent_add(Editor *e, const char *path) {
  ...
  for (i = 0; i < e->nrecent; i++) {
    if (strcmp(e->recent[i], path) == 0) {
      char tmp[1024];
      snprintf(tmp, sizeof tmp, "%s", e->recent[i]);
      memmove(e->recent[1], e->recent[0], (size_t)i * sizeof e->recent[0]);
      snprintf(e->recent[0], sizeof e->recent[0], "%s", tmp);
      return;
    }
  }
  ...
  snprintf(e->recent[0], sizeof e->recent[0], "%s", path);
}
...
static bool save_to(Editor *e, Doc *d, const char *path) {
  if (!path || !path[0]) {
...
static bool match_at(Editor *e, Doc *d, size_t i, size_t flen) {
  size_t len = buf_len(&d->buf);
  if (i + flen > len) return false;
  if (e->find_case) {
    if (!buf_match(&d->buf, i, e->find, flen)) return false;
  } else {
    if (!buf_match_ci(&d->buf, i, e->find, flen)) return false;
  }
  if (e->find_word) {
    if (i > 0 && is_word((unsigned char)buf_at(&d->buf, i - 1))) return false;
...
static void do_replace_all(Editor *e, Doc *d) {
  ...
  while (i + flen <= buf_len(&d->buf)) {
    ...
    if (rlen && !push_insert(e, d, i, e->replace, rlen, false)) {
      set_status(e, "replace aborted");
      break;
    }
    count++;
  ...
  {
    char msg[64];
    snprintf(msg, sizeof msg, "replaced %zu", count);
    set_status(e, msg);
  }
  mark(e);
}
...
static void indent_sel(Editor *e, Doc *d, int dir) {
  ...
  for (row = r0; row <= r1; row++) {
    pos = row_start(d, row);
    if (dir > 0) {
      if (!push_insert(e, d, pos, "\t", 1, false)) break;
      if (a >= pos) a++;
      b++;
    } else {
      char c0;
...
static void dup_line(Editor *e, Doc *d) {
  ...
  if (b < buf_len(&d->buf) && buf_at(&d->buf, b) == '\n') {
    ...
    s = slice_dup(d, a, b);
    if (!s) return;
    if (!push_insert(e, d, b, s, n, false)) {
      free(s);
      return;
    }
    free(s);
    ...
    if (n && !push_insert(e, d, b + 1, s, n, false)) {
      free(s);
      return;
    }
    free(s);
...
static void cycle_theme(Editor *e) {
  ...
  {
    char msg[64];
    snprintf(msg, sizeof msg, "theme: %s", theme_name(e->theme_id));
    set_status(e, msg);
  }
  mark(e);
}
...
static void paste_clip(Editor *e, Doc *d, Plat *p) {
  ...
  delete_sel(e, d);
  push_insert(e, d, d->caret, s, n, false);
  d->caret += n;
  clear_sel(d);
  sync_caret_rc(d);
  ensure_visible(e, d);
...
static bool doc_init_empty(Doc *d) {
  doc_reset(d);
  if (!buf_init(&d->buf, 0)) return false;
  undo_init(&d->undo);
  return true;
}
...
bool ed_init(Editor *e) {
  ...
  e->ndocs = 1;
  e->cur = 0;
  if (!doc_init_empty(D(e))) return false;
  e->theme_id = 0;
...
static void switch_doc(Editor *e, int idx) {
  ...
  {
    char msg[48];
    snprintf(msg, sizeof msg, "doc %d/%d", e->cur + 1, e->ndocs);
    set_status(e, msg);
  }
  mark(e);
}
...
static void close_doc(Editor *e) {
  ...
  if (d->dirty) {
    ...
    set_status(e, "Unsaved! Ctrl+S save+close, Ctrl+Q discard+close, Esc");
    return;
  }
  close_doc_force(e);
}
...
static void finish_quit_saves(Editor *e) {
  ...
  e->quit_after_save = false;
  if (!any_dirty(e)) e->want_quit = true;
}
...
static void jump_bracket(Editor *e, Doc *d) {
  ...
  if (d->caret == d->bracket_b)
    move_caret(e, d, d->bracket_a, false);
  else
    move_caret(e, d, d->bracket_b, false);
}
...
static void prompt_enter(Editor *e) {
  ...
  if (e->mode == MODE_OPEN) {
    ...
    snprintf(e->find, sizeof e->find, "%.255s", e->prompt);
    e->mode = MODE_EDIT;
    find_next(e, d);
  } else if (e->mode == MODE_REPLACE) {
    snprintf(e->replace, sizeof e->replace, "%.255s", e->prompt);
    e->mode = MODE_EDIT;
    do_replace_all(e, d);
...
void ed_handle(Editor *e, Plat *p, const PlatEvent *ev) {
  ...
  case PK_FIND:
    ...
    set_status(e, "Find — type, Enter (Alt+C case, Alt+W word)");
    break;
  ...
  case PK_REPLACE:
    e->mode = MODE_REPLACE;
    e->prompt[0] = 0;
    set_status(e, "Replace all with — type, Enter");
    break;
  case PK_GOTO:
    e->mode = MODE_GOTO;
    e->prompt[0] = 0;
    set_status(e, "Go to line — type, Enter");
    break;
...
void ed_draw(Editor *e, Plat *p) {
  ...
  static const char *help[] = {
      ...
      "  Ctrl+O / Q / F5      open / quit / reload",
      "  Ctrl+N / Shift+W     new / close doc",
      "  Ctrl+Tab / S-Tab     next / prev doc",
      "  Ctrl+E               recent files",
      "",
      "Edit",
      "  Ctrl+Z / Y           undo / redo",
      "  Ctrl+F   F3 / S-F3   find · next / prev",
      "  Alt+C / W · Ctrl+R   case / word · replace",
      "  Ctrl+G / D / ] / T   line · dup · bracket · theme",
      "  Ctrl+Shift+K         delete line",
      "  Tab / Shift+Tab      indent / outdent",
      "  Ctrl+X / C / V / A   cut copy paste all",
      "",
      "View",
      "  Ctrl+W · F7          wrap · whitespace",
      "  Ctrl+= / - / 0       zoom",
      "  Ctrl+Shift+R / E     readonly · EOL",
      "",
      "Also   syntax · line nums · match highlight",
      "Unsaved / dirty open   Ctrl+S · Ctrl+Q · Esc",
  };
  ...
  plat_clear(p, t->bg);
  if (e->gutter > 0)
    plat_fill_rect(p, 0, 0, e->gutter, e->rows * e->ch, t->gutter_bg);
  ...
  for (i = 0; i < e->rows; i++) {
    ...
    if (wseg == 0) {
      snprintf(num, sizeof num, "%*zu", digits, lrow + 1);
      plat_draw_text(p, 2, i * e->ch, num, (int)strlen(num), t->gutter_fg);
    }
    ncopy = (int)(b - a);
    if (ncopy > (int)sizeof linebuf - 1) ncopy = (int)sizeof linebuf - 1;
    {
  ...
  if (e->mode != MODE_HELP && e->mode != MODE_RECENT && cvr >= top &&
    ...
    if (ok) {
      ...
      if (!plat_set_caret(p, cx, cy, e->ch, true))
        plat_fill_rect(p, cx, cy, 2, e->ch, t->caret);
    } else
  ...
  sw = e->rows * e->ch;
  plat_fill_rect(p, 0, sw, w, e->ch, t->status);
  ...
  if (e->mode == MODE_OPEN || e->mode == MODE_SAVEAS || e->mode == MODE_FIND ||
    ...
    snprintf(bar, sizeof bar, "[%d/%d] %s%s%s  %zu:%zu  %s  %s  %s", e->cur + 1,
             e->ndocs, base, d->dirty ? "*" : "", d->readonly ? " RO" : "",
             crow + 1, ccol + 1, d->eol == EOL_CRLF ? "CRLF" : "LF",
             hl_lang_name(syn), e->status[0] ? e->status : "F1 help");
  }
  plat_draw_text(p, 4, sw, bar, (int)strlen(bar), t->status_fg);

  if (e->mode == MODE_HELP || e->mode == MODE_RECENT) {
    int nlines_h, box_h, box_y, box_w;
    if (e->mode == MODE_HELP) {
      nlines_h = (int)(sizeof help / sizeof help[0]);
      box_h = (nlines_h + 2) * e->ch;
      box_y = e->ch;
      box_w = w - 40;
      if (box_w < 200) box_w = w - 8;
      plat_fill_rect(p, 16, box_y - 4, box_w + 8, box_h + 8, t->help_bd);
      plat_fill_rect(p, 20, box_y, box_w, box_h, t->help_bg);
      for (i = 0; i < nlines_h; i++)
        plat_draw_text(p, 28, box_y + (i + 1) * e->ch, help[i],
                       (int)strlen(help[i]), t->fg);
    } else {
      nlines_h = e->nrecent + 2;
      box_h = (nlines_h + 2) * e->ch;
      box_y = e->ch;
      box_w = w - 40;
      if (box_w < 200) box_w = w - 8;
      plat_fill_rect(p, 16, box_y - 4, box_w + 8, box_h + 8, t->help_bd);
      plat_fill_rect(p, 20, box_y, box_w, box_h, t->help_bg);
      plat_draw_text(p, 28, box_y + e->ch, "Recent files", 12, t->fg);
      for (i = 0; i < e->nrecent; i++) {
        char line[300];
        const char *rp = e->recent[i];
        const char *slash = strrchr(rp, '/');
        const char *b2 = strrchr(rp, '\\');
        if (b2 && (!slash || b2 > slash)) slash = b2;
        snprintf(line, sizeof line, "%s%d %s", i == e->recent_sel ? "> " : "  ",
                 i + 1, slash ? slash + 1 : rp);
        plat_draw_text(p, 28, box_y + (i + 2) * e->ch, line, (int)strlen(line),
                       i == e->recent_sel ? t->kw : t->fg);
      }
    }
  }

  {
    ...
    char title[260];
    const char *base = d->path[0] ? d->path : "untitled";
    const char *slash = strrchr(base, '/');
    const char *b2 = strrchr(base, '\\');
    if (b2 && (!slash || b2 > slash)) slash = b2;
    if (slash) base = slash + 1;
    snprintf(title, sizeof title, "%s%s — mote-x", d->dirty ? "*" : "", base);
