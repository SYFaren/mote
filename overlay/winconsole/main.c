...
static void usage(void) {
  fprintf(stderr,
          ...
          "Windows console; Ctrl+Q quit, F1 help; %%APPDATA%%\\%s\\config\n",
          MOTE_NAME, MOTE_NAME);
}
...
static int parse_geom(const char *s, int *w, int *h) {
  ...
  if (a < 40 || b < 10 || a > 300 || b > 120) return -1;
  *w = a;
  *h = b;
  return 0;
}
...
int main(int argc, char **argv) {
  ...
  cfg_load(&cfg);
  if (cfg.win_w > 300 || cfg.win_h > 120 || cfg.win_w < 40 || cfg.win_h < 10) {
    cfg.win_w = 0;
    cfg.win_h = 0;
  }
  ...
  if (!plat) {
    fprintf(stderr, "%s: need a console\n", MOTE_NAME);
    return 1;
  }
  if (!ed_init(&ed)) {
    plat_destroy(plat);
